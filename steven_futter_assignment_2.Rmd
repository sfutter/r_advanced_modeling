...
Using log(price) as the response the forward variable selection algorithm and evaluating by adjusted R^2 produces a 9 variable model including the following variables:
...
The backward variable selection algorithm produces a 10 variable model which includes the following variables:
 1.  carat, 
 2.  color, 
 3.  clarity, 
 4.  channelInternet, 
...
log(price) ~ carat + color + clarity + channelInternet + channelMall + storeBlue Nile + storeFrey Meyer + storeGoodmans + storeR. Holland + storeRiddles. 
...
```{r echo=FALSE, message=FALSE}
regfit.stepwise.log = regsubsets(log.price~., data=d.train, nvmax = 17, method='seqrep', nbest=1)
